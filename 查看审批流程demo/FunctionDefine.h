...
CG_INLINE NSString*  ShiftThetTimestampByString(NSString *time)
{
    ...
    [dateFormatter setDateFormat:iStoreLocalizedString(@"MM月dd日 HH:mm")];
    ...
    if (Subtime<=60) {  //// 1分钟以内的
        dateStr = iStoreLocalizedString(@"刚刚");
    }else if(Subtime<=60*60){ // 一个小时以内的
        
        int mins = Subtime/60;
        dateStr = [NSString stringWithFormat:@"%d%@",mins,iStoreLocalizedString(@"分钟前")];
        ...
        if ([need_yMd isEqualToString:now_yMd]) {
            //// 在同一天
            dateStr = [NSString stringWithFormat:@"%@%@",iStoreLocalizedString(@"今天"),[dateFormatter stringFromDate:needFormatDate]];
        }else{
            ////  昨天
            dateStr = [NSString stringWithFormat:@"%@%@",iStoreLocalizedString(@"昨天"),[dateFormatter stringFromDate:needFormatDate]];
        }
    }else {
        ...
        if ([yearStr isEqualToString:nowYear]) {
            ////  在同一年
            [dateFormatter setDateFormat:iStoreLocalizedString(@"MM月dd日")];
